...
def apply_job(job_id):
    ...
    if form.validate_on_submit():
        ...
        secure_name = f"{uid}_{filename}"
        upload_path = current_app.config['UPLOAD_FOLDER']
        try:
            f.save(os.path.join(upload_path, secure_name))
            current_app.logger.info(f"Resume saved: {secure_name}")
        except Exception as e:
            current_app.logger.error(f"Resume save error: {e}")
            flash("Resume upload error.", 'danger')
            return render_template('jobs/detail.html', title=job.title, job=job, already_applied=False, form=form)
        ...
        app_record = Application(
            ...
            earliest_join_date=form.earliest_join_date.data, resume_filename=secure_name, status='Submitted'
        )
        ...
        try:
            ...
            now_time = datetime.utcnow() # Get time once for emails
            ...
            try:
                os.remove(os.path.join(upload_path, secure_name))
                current_app.logger.warning(f"Deleted orphaned resume {secure_name}.")
            except OSError: pass
...
@employer_required
def edit_job(job_id):
    # --- V V V --- CORRECTED INDENTATION --- V V V ---
    job = Job.query.get_or_404(job_id)
    # Security check - MUST be indented correctly relative to `job = ...`
    if job.employer_id != current_user.id:
        abort(403)
    # Form creation - MUST align with the `if` above
    form = JobForm(obj=job)
    # Validation block - MUST align
    if form.validate_on_submit():
        # Inside if - indented further
        ...
        # Return redirect - indented inside the 'if form.validate...' block
        return redirect(url_for('employers.dashboard'))
    # Final return - MUST align with the 'if form.validate...' block
...
def download_resume(filename):
    app_record = Application.query.filter_by(resume_filename=filename).first_or_404("Record not found.")
    job = Job.query.get_or_404(app_record.job_id)
    is_owner = current_user.is_authenticated and current_user.role == 'employer' and job.employer_id == current_user.id
    is_admin = current_user.is_authenticated and current_user.role == 'admin'
    if not (is_owner or is_admin):
        current_app.logger.warning(f"Unauthorized resume download: user {current_user.id}, file {filename}")
        abort(403)
    upload_folder = current_app.config['UPLOAD_FOLDER']
    current_app.logger.info(f"Download resume: user {current_user.id}, file {filename}")
    try:
        return send_from_directory(directory=upload_folder, path=filename, as_attachment=True)
    except FileNotFoundError:
        current_app.logger.error(f"Resume not found: {os.path.join(upload_folder, filename)}")
        abort(404)
    except Exception as e:
        current_app.logger.error(f"Resume download error: {e}")
        abort(500)
...
def reject_application(application_id):
    ...
    if job.employer_id != current_user.id:
        ...
        abort(403)
    if application.status not in ['Submitted', 'Viewed', 'Shortlisted', 'Interviewing']: # Allow reject from intermediate stages
