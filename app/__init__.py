...
def create_app(config_class=None):
    ...
    # --- V V V --- Simplified Upload Folder Logic --- V V V ---
    # Default local path inside 'instance'
    default_upload_folder = os.path.join(app.instance_path, 'uploads', 'resumes')
    # Use environment variable if set (set this on Render), otherwise use default
    ...
    app.config.from_mapping(
        ...
        # --- Use the calculated upload_folder_path ---
    ...
    try:
        # Use parents=True to create intermediate dirs like 'uploads' if needed
        ...
        # Optional: Check writability if errors persist
        # if not os.access(app.config['UPLOAD_FOLDER'], os.W_OK): app.logger.error(f"Upload dir {app.config['UPLOAD_FOLDER']} NOT WRITABLE.")
         ...
         app.logger.error(f"PERMISSION DENIED creating/accessing upload directory {app.config['UPLOAD_FOLDER']}. Check Render Disk mount point and permissions. Error: {pe}")
    ...
    def load_user(user_id): from .models import User; try: return User.query.get(int(user_id)); except Exception as e: app.logger.error(f"Error loading user {user_id}: {e}"); return None
    ...
    with app.app_context():
        ...
        except Exception as e: app.logger.error(f"Error db.create_all(): {e}"); app.logger.error(f"DB URI: {app.config.get('SQLALCHEMY_DATABASE_URI')}")
